...
remainingLettersSet = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"]
...
gameContinues = true
  roundContinues = true
  while roundContinues == true:
  print("Please guess a letter.")
  for secretWord[i]:
    if secretWord[i] is in remainingLettersSet:
      print(" _ ")
    else:
      print (secretWord[i])
    #Ask for the next letter and make it lowercase
    guessedLetter = input()
    lowerGuess = lower(guessedLetter)
    #If the "letter" has multiple characters Notify the player that the "letter" has multiple characters
    if len(lowerGuess) > 1:
    print("Please guess only one letter at a time.")
    #Else if the letter is not an ASCII lowercase character Notify the player that the letter is not an ASCII lowercase character
    elif lowerGuess is not in lowerAlphabet:
    print("Please guess a letter.")
    #Else if the letter is not in the remaining letter set (i.e. has been guessed before) Notify the player that the letter has been guessed before
    elif lowerGuess is not in remainingLettersSet:
    ...
    elif lowerGuess is in secretWord:
